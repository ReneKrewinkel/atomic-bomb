...
test("getInstalledSkillDir resolves versioned target skill directory", () => {
  assert.equal(
    getInstalledSkillDir({ appVersion: "1.2.3", projectDir: "." }),
    ".skills/atomic-bomb/1.2.3",
  );
});

test("getInstalledSkillIndex resolves versioned target skill index", () => {
  assert.equal(
    getInstalledSkillIndex({ appVersion: "1.2.3", projectDir: "." }),
    ".skills/atomic-bomb/1.2.3/index.md",
  );
});
...
test("installSkillBundle copies bundled skills into the target project", () => {
  ...
  fs.mkdirSync(
    path.join(projectDir, ".skills/atomic-bomb/1.2.3/stale"),
    { recursive: true },
  );
  fs.writeFileSync(
    path.join(projectDir, ".skills/atomic-bomb/1.2.3/stale/old.md"),
    "# Old\n",
  );
  ...
  assert.equal(
    installedIndex,
    path.join(projectDir, ".skills/atomic-bomb/1.2.3/index.md"),
  );
  assert.equal(fs.existsSync(installedIndex), true);
  assert.equal(
    fs.existsSync(
      path.join(projectDir, ".skills/atomic-bomb/1.2.3/nested/rule.md"),
    ),
    true,
  );
  assert.equal(
    fs.existsSync(path.join(projectDir, ".skills/atomic-bomb/1.2.3/.DS_Store")),
    false,
  );
  assert.equal(
    fs.existsSync(
